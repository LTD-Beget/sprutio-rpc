...
class CopyToWebDav(BaseWorkerCustomer):
    ...
    def run(self):
        try:
            ...
            self.logger.debug("CopyToWebDav process run source = %s , target = %s" % (source_path, target_path))
    ...
    def uploading_progress(self, download_t, download_d, upload_t, upload_d):
        try:
            ...
            self.logger.error("Error in CopyToWebDav uploading_progress(): %s, traceback = %s" %
