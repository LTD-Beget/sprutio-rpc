...
class CopyBetweenSftp(BaseWorkerCustomer):
    def __init__(self, source, target, paths, overwrite, *args, **kwargs):
        super(CopyBetweenSftp, self).__init__(*args, **kwargs)
                ...
                try:
                    ...
                    if source_sftp.isdir(abs_path):
                        ...
                        for current, dirs, files in source_sftp.walk(abs_path):
                            ...
                            for f in files:
                                ...
                                if not target_sftp.exists(target_file):
                                    self.copy_between_sftp(source_sftp, target_sftp, source_file, target_file)
                        ...
                        try:
                            ...
                            if not target_sftp.exists(target_file):
                                self.copy_between_sftp(source_sftp, target_sftp, abs_path, target_file)
                            elif self.overwrite and target_sftp.exists(target_file) and not target_sftp.isdir(
                                    target_file):
                                target_sftp.remove(target_file)
                                self.copy_between_sftp(source_sftp, target_sftp, abs_path, target_file)
    ...
    @staticmethod
    def copy_between_sftp(source_sftp, target_sftp, source_path, target_path):
        """
